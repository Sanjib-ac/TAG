...
class SARDecoder(nn.Module):
    ...
    def forward(self, encoder_outputs, targets=None):
        ...
        Returns:
            outputs: Tensor of shape [B, max_seq_len, num_classes] (logits per time step)
        """
...
class CustomOCRDataset(Dataset):
    ...
    Expects a CSV file with columns:
      - 'image': filename of the image (located in img_dir)
      - 'label': string representing the 10-digit number (e.g., "1234567890")
    """
    ...
    def __getitem__(self, idx):
        ...
        # Convert label string into a list of integers.
        label_str = row['label'].strip()
        label = [int(ch) for ch in label_str]  # For a 10-digit sequence
...
# -------------------------------------------------------------------
def train(args):
    # Define a comprehensive transformation pipeline with data augmentations and normalization.
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406],  # These are ImageNet normalization stats.
    ...
    # Optionally, freeze the detection module if you want to fine-tune only the recognition parts.
    for param in model.det_predictor.parameters():
        param.requires_grad = False

    # Patch the recognition submodule with our custom SARDecoder.
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # Convert tensor images into PIL images for DocumentFile.
            to_pil = transforms.ToPILImage()
            batch_images = [to_pil(img.cpu()) for img in images]
            doc = DocumentFile.from_images(batch_images)

            # Forward pass: extract encoder features and run custom decoder.
            features = model.reco_predictor.encoder(doc)  # [B, seq_len_enc, hidden_size]
            logits = model.reco_predictor.decoder(features)  # [B, seq_len, num_classes]

            # Reshape logits and labels to compute loss (treat each time step as a separate classification).
            B, T, C = logits.size()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_save_path', type=str, default='model_finetuned.pth',
                        help='Path to save the trained model')
    parser.add_argument('--print_freq', type=int, default=10, help='Print frequency (batches) during training')
